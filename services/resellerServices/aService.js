...
exports.deleteAddressRecord = async ({ domainName, host, addressValue }) => {
  try {
    const response = await axios.post(
      'https://test.httpapi.com/api/dns/manage/delete-ipv4-record.json',
      null,
      {
        params: {
          'auth-userid': process.env.AUTH_USERID,
    ...
    throw new Error('Error deleting Address(IP) record: ' + error.message);
  }
};