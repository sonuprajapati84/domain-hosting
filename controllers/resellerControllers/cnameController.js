const cnameService = require('../../services/resellerServices/cnameService');
...
exports.deleteCnameRecord = async (req, res) => {
    const { domainName, host, cnameValue } = req.body;
    if (!domainName || !host || !cnameValue ) {
        return res.status(400).json({
            message: 'All Fields are required { domainName, host, cnameValue }',
        });
    }
    try {
        const response = await cnameService.deleteCnameRecord({ domainName, host, cnameValue });
        if (response.status === 'Success') {
            return res.status(200).json({
        ...
        console.error('Error deleting CNAME record:', error.response?.data || error.message);
        return res.status(500).json({
            message: 'Error deleting CNAME record',
            error: error.response?.data?.message || error.message,
        });
    }
};