const mxService = require('../../services/resellerServices/mxService');
...
exports.deleteMxRecord = async (req, res) => {
    const { domainName, host, mxValue } = req.body;
    if (!domainName || !host || !mxValue ) {
        return res.status(400).json({
            message: 'All Fields are required { domainName, host, mxValue }',
        });
    }
    try {
        const response = await mxService.deleteMxRecord({ domainName, host, mxValue });
        if (response.status === 'Success') {
            return res.status(200).json({
                message: 'MX record deleted successfully.',
                details: response,
            });
        } else {
            return res.status(400).json({
        ...
        console.error('Error deleting MX record:', error.response?.data || error.message);
        return res.status(500).json({
            message: 'Error deleting MX record',
            error: error.response?.data?.message || error.message,
        });
    }
};